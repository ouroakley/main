...
**Tier B — recurring in the same calendar month (hall timetables, weekly slots on `main`):** Use **one** outer content directory per activity per month (e.g. `…-april-2026` or `…-may-2026`) with a **single** `index.md` under the **first occurrence’s** inner date folder (`<yyyy-mm-dd>/index.md`). Put **every** instance that month in one `eventDates` list (sorted chronologically). **Do not** create parallel sibling folders `content/events/<short-slug>/<weekly-date>/` for the same recurring activity in the same month—that pattern is deprecated on `main` (use one monthly bundle + `aliases` if replacing old weekly URLs).
